...
#寻找密度吸引子
def findAttractor(x,D,h,minRange):
    ...
    for i in range(len(D)):
        x_t_next += (GaussianKernel((x_t - D[i])/h) * D[i])
        GaussianSum += GaussianKernel((x_t - D[i])/h)
    ...
    for a in dis_list:
        return_count += a
    # for Num in range(30):
    #     if (len(dis_list) < Num+1):
    #         break
    #     return_count += dis_list[Num]
    return  x_t_next,return_count

def DENCLUE(D,h,minValue,minRange,Label):
    ...
    for eachVectorPoint in D:
        x_attractor,lastTwoStep = findAttractor(eachVectorPoint,D,h,minRange)
        ...
        if ((SumGaussian / (len(D) * math.pow(h,len(eachVectorPoint)))) >= minValue):
...
if __name__=="__main__":
    ...
    #     print(D[i])
    #调用DENCLUE方法进行聚类
    eachCulsterAttractors,clusterResultSet,clusterResultLabel = DENCLUE(D,0.25,0.25,0.0001,Label)
